# ms-sg profile (summary)

**Language:** `malay` · **Overlay id:** `ms-sg`

Full rules: [languages/malay/overlays/ms-sg.md](../languages/malay/overlays/ms-sg.md).

Malay overlay — load when `regional_voice: ms-sg` or user names this region/variety.

**When to load:** `language: malay` + `regional_voice: ms-sg`.
