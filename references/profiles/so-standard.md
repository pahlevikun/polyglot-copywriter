# so-standard profile (summary)

**Language:** `somali` · **Overlay id:** `so-standard`

Full rules: [languages/somali/overlays/so-standard.md](../languages/somali/overlays/so-standard.md).

Somali overlay — load when `regional_voice: so-standard` or user names this region/variety.

**When to load:** `language: somali` + `regional_voice: so-standard`.
...
**Locale techniques:** [locales/somali.md](../techniques/locales/somali.md) when use case matches.
